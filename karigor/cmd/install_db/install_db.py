...
def install_db():
    ...
    drivers_to_install = questionary.checkbox(
        ...
        choices=[
            questionary.Choice(
                ...
                description="PostgreSQL adapter for standard (synchronous) operations",
                checked=False
            ),
            questionary.Choice(
                "MySQL (pymysql)", 
                value="pymysql",
                description="MySQL adapter for standard (synchronous) operations",
                checked=False,
            ),
        ],
    ).ask()
    ...
    console.print(
        f"[bold blue]⏳ Installing packages: {', '.join(final_packages)}...[/bold blue]"
    )
    ...
    def write_from_template(template_name, output_path):
        ...
        if os.path.exists(template_path):
            ...
            console.print(
                f"[bold red]❌ Error: Template file '{template_name}' not found at {template_path}![/bold red]"
            )
    ...
    try:
        ...
        if not os.path.exists("alembic"):
            ...
            console.print(
                "[bold green]✔[/bold green] Database tools and selected drivers installed successfully!"
            )
        else:
            console.print("[yellow]⚠ Alembic is already initialized.[/yellow]")
            write_from_template("alembic_env.txt", "alembic/env.py")
            console.print(
                "[bold green]✔[/bold green] Selected drivers installed and env.py configurations verified successfully!"
            )
